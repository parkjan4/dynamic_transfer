...
class EventCalendar():
    ...
    def __init__(self):
        self.ThisCalendar = [] # Basically the list of pending events
    ...
    def Schedule(self,addedEvent):
        ...
        if len(self.ThisCalendar) == 0:  #no events in calendar
            ...
            # Since addedEvent is the last in terms of time, append to end
            self.ThisCalendar.append(addedEvent)
        else:
            # We need to find where addedEvent fits in terms of time
...
class Resource():
    ...
    def Free(self, Units):
        ...
        if diff < 0:
            free = False # Means we tried to free more resource than available
        else:
            self.Busy = self.Busy - Units
            self.NumBusy.Record(float(self.Busy))
            free = True
