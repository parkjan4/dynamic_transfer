...
def lcgrandst(ZRNG, zset,Stream):
    ZRNG[Stream-1] = zset
    

def lcgrandgt(ZRNG, Stream):
    return ZRNG[Stream-1]


def Expon(ZRNG, Mean, Stream):
    Mean = float(Mean)
    return -math.log(1 - lcgrand(ZRNG, Stream)) * Mean
    
    
def Uniform(ZRNG, Lower, Upper, Stream):
    Lower = float(Lower)
    Upper = float(Upper)
    return Lower + (Upper - Lower) * lcgrand(ZRNG, Stream)
    
    
def Random_integer(ZRNG, prob_distrib, Stream):
    U = lcgrand(ZRNG, Stream)
    random_integer = 1
...
def Erlang(ZRNG, m, Mean, Stream):
    Mean = float(Mean)
    mean_exponential = Mean / m
    Sum = 0.0
    for i in range(0,m,1):
        Sum = Sum + Expon(ZRNG, mean_exponential, Stream)
    erlang = Sum
    return erlang
    
    
def Triangular(ZRNG, a, b, c, Stream):
    a = float(a)
    b = float(b)
    c = float(c)
    Standardb = (b - a) / (c - a)
...
def Normal(ZRNG, Mean, Variance, Stream):
    Mean = float(Mean)
    Variance = float(Variance)
    U1 = lcgrand(ZRNG, Stream)
    U2 = lcgrand(ZRNG, Stream)
    V1 = 2 * U1 - 1
    V2 = 2 * U2 - 1
    W = V1 ** 2 + V2 ** 2
    while (W > 1):
...
def Lognormal(ZRNG, MeanPrime, VariancePrime, Stream):
    ...
    lognormal = math.exp(Normal(ZRNG, Mean, Variance, Stream))
    return lognormal